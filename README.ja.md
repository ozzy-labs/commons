...
3. org-level secrets として `SYNC_APP_ID` と `SYNC_APP_PRIVATE_KEY` をリポから参照可能にする。これらは org にインストールされた GitHub App のもので、`contents: write` / `pull-requests: write` / `workflows: write` 権限を持つ必要がある。デフォルトの `GITHUB_TOKEN` には `workflows` 権限がなく `.github/workflows/*` を含む差分を push できないため、App token に切り替えている
4. 翌週から scheduled で自動起動。`workflow_dispatch` で即時実行も可能
